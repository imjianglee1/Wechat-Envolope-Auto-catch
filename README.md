...
> ⚠️ **免责声明**  
> 本项目仅供学习与参考，请勿用于扰乱微信团队、群聊良好氛围或社会良好风气等不良用途。  
> 使用本脚本产生的任何后果，作者不承担任何责任。
...
如有问题或建议，欢迎通过邮件联系：  
📧 **imjianglee@outlook.com**

---

## 📖 项目简介

本项目是一个基于 **OCR 图像识别** 的自动抢红包脚本，使用 Python、OpenCV、PyAutoGUI 等技术实现全自动操作。  
程序能够自动识别屏幕中的红包图标，并完成点击、拆包、返回等一系列操作，全程无需人工干预。

> 💡 本项目使用 DeepSeek 工具进行辅助创作。
...
1. **第一阶段**：循环匹配红包模板（阈值 ≥ 0.8），点击后进入下一阶段
2. **第二阶段**：启动计时器，若在超时时间（默认 1 秒）内检测到“打开”按钮则点击，否则自动跳过
3. **第三阶段**：检测“返回”按钮并点击，重新开始循环
...
### 1. 准备模板图片
将以下模板图片放置在项目目录中（可从 GitHub 附件下载）：
...
> 📎 模板图片已上传至 GitHub 附件，请自行下载并放置到对应目录。
...
### 效果演示
![运行截图](https://github.com/user-attachments/assets/dc52d34e-17c0-4371-a4b7-8ce8282b5aea)

### 样例视频
[点击查看演示视频](https://github.com/user-attachments/assets/e6786f5a-41e8-4820-beae-694dca60e6a9)
